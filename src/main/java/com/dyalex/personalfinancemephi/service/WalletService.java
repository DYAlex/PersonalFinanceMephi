    ...
    private List<Wallet> setInBound(List<Wallet> wallets) {
        return wallets.stream()
                .peek(wallet -> wallet.setLimits(wallet.getLimits().stream()
                        ...
                        .peek(limit -> limit.setInBound(limit.getBalance().compareTo(BigDecimal.ZERO) >= 0))
                                .peek(limit -> System.out.println("Limit: InBound " + limit.getInBound() + " " + limit.getBalance() + " " + limit.getValue()))
                        .toList()))
